...
class PointDecoder(nn.Module):
    ...
    def forward(self, image_embeddings: torch.Tensor, points: torch.Tensor, masks: torch.Tensor = None):
        """
        Generate heatmap (in training mode), predict points

        Params
        ------
            image_embeddings: the result after sam's image encoder of size (N, 256, 64, 64).
            points: the annotation points provided of shape (N, M, 2). M is specified by `max_
            points` in `data.py`, default to 500.
            masks: pseudo masks generated by early-phase Sam.
